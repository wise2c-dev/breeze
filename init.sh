...
docker run --rm --name=kubeadm-version wisecloud/kubeadm-version:${kubernetes_version} kubeadm config images list --kubernetes-version ${kubernetes_version} --config=kube-dns.yaml > ${path}/k8s-images-list.txt
